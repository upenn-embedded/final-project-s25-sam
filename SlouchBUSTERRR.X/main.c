...
// Initialize Timer1 in normal mode with 1탎 resolution
void timer1_init() {
    TCCR1A = 0;
    TCCR1B = (1 << CS11);  // Prescaler = 8 (1 tick = 0.5 탎 at 16MHz)
    TIMSK1 = (1 << ICIE1); // Enable input capture interrupt
    //Initialize abc
    TCCR1B |= (1 << ICES1); // Start with Rising Edge
}

// Send 10탎 pulse on TRIG_PIN
void send_trigger_pulse() {
    DDRB |= (1 << TRIG_PIN);  // Set TRIG_PIN as output
    PORTB &= ~(1 << TRIG_PIN); // Ensure it's LOW
    _delay_us(2);
    PORTB |= (1 << TRIG_PIN);  // Set HIGH for 10탎
    _delay_us(10);
    ...
    if (TCCR1B & (1 << ICES1)) {  // Rising edge detected
        ...
        TCCR1B &= ~(1 << ICES1);  // Switch to falling edge detection
         //  Compute Distance Outside ISR
            uint16_t duration = timer_count / 2; // Convert to 탎 (each tick = 0.5탎)
