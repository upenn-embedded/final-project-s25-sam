...
#define TRIG_PIN PB0        // New trigger pin
#define ECHO_PIN PB1        // Keep echo pin the same
...
void send_trigger_pulse() {
    DDRB |= (1 << TRIG_PIN);           // TRIG as output
    PORTB &= ~(1 << TRIG_PIN); _delay_us(2);  // Low for 2us
    PORTB |= (1 << TRIG_PIN); _delay_us(10);  // High for 10us
    PORTB &= ~(1 << TRIG_PIN);                // Low again
    printf("Pulse sent.\n");
}
...
ISR(TIMER1_CAPT_vect) {
    ...
    if (TCCR1B & (1 << ICES1)) {
        start_time = ICR1;
        printf("Echo rising detected.\n");
        TCCR1B &= ~(1 << ICES1);
    } else {
        timer_count = ICR1 - start_time;
        measurementReady = 1;
        printf("Echo falling detected. Pulse width: %u\n", timer_count);
        TCCR1B |= (1 << ICES1);
    }
}
